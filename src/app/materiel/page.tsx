        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="p-5 border-t border-slate-100 flex gap-3 shrink-0 bg-slate-50/50">
              ...
              <button onClick={save} disabled={!form.categorie || (form.categorie === 'AUTRE' && !form.categorieAutre.trim())}
                className="flex-1 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-purple-700 text-white font-bold hover:from-violet-700 hover:to-purple-800 disabled:opacity-50 shadow-md">
              ...
              <div key={m._id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-1 ml-2 shrink-0">
                    ...
                    <button onClick={() => remove(m._id)} className="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <button onClick={() => toggleEtat(m)}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${isFonc ? 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' : 'bg-rose-50 text-rose-700 hover:bg-rose-100'}`}>
                    {isFonc ? <CheckCircle size={13} /> : <AlertTriangle size={13} />}
