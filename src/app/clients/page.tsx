...
import { Plus, Search, MoreVertical, Users, Phone, PenTool, Upload, Globe2, Building2, Layers, X, Pencil, Trash2, Save, ChevronDown } from 'lucide-react';
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {loading ? (
            ...
            <div className="px-6 py-16 text-center">
              ...
              <h3 className="font-bold text-slate-700">Aucun client trouvé</h3>
              <p className="text-slate-400 text-sm mt-1">Essayez une autre recherche.</p>
            </div>
          ) : (
            <div className="p-4 space-y-3">
              {displayedClients.map((client) => {
                const expanded = expandedClientId === client._id;
                const services = client.servicesUtilises || [];
                return (
                  <article key={client._id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-100 to-indigo-100 text-indigo-700 flex items-center justify-center text-xs font-black border border-indigo-200 shrink-0">
                            {client.prenom?.[0]}{client.nom?.[0]}
                          </div>
                          <div className="min-w-0">
                            <div className="font-bold text-slate-800 truncate">{client.prenom} {client.nom}</div>
                            <div className="text-xs text-slate-500 truncate">{client.email}</div>
                            <div className="mt-1 inline-flex items-center rounded-full bg-white px-2 py-0.5 text-[11px] font-semibold text-slate-600 border border-slate-200">
                              {getCountryDisplayName(client.paysResidence || 'INCONNU')}
                            </div>
                          </div>
                        </div>
                      </div>
                      <button
                        onClick={() => setExpandedClientId(expanded ? null : client._id)}
                      ...
                      <div className="mt-4 space-y-3">
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="rounded-xl bg-white border border-slate-200 p-3">
                            <div className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Téléphone</div>
                            <div className="mt-1 text-slate-700 font-medium">{client.telephone || '—'}</div>
                          </div>
                          <div className="rounded-xl bg-white border border-slate-200 p-3">
                            <div className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold">Type</div>
                            <div className="mt-1 text-slate-700 font-medium">{client.typeClient || 'PARTICULIER'}</div>
                          </div>
                        </div>

                        <div className="rounded-xl bg-white border border-slate-200 p-3">
                          <div className="text-[11px] uppercase tracking-wide text-slate-400 font-semibold mb-2">Services</div>
                          <div className="flex flex-wrap gap-2">
                            {services.length > 0 ? services.map((service: string) => (
                              <span key={service} className="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-semibold text-blue-700 border border-blue-100">
                                {service}
                              </span>
                        ...
                        <div className="flex gap-2 pt-1">
                          <button
                            ...
                            className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-bold text-white"
                          >
                            <Pencil size={14} /> Modifier
                          </button>
                          <button
                            onClick={() => handleDelete(client._id)}
                            className="inline-flex items-center justify-center rounded-xl border border-rose-200 bg-white px-4 py-2.5 text-sm font-bold text-rose-600"
                          >
                            <Trash2 size={14} />
                          </button>
                        </div>
              ...
              {!loading && filteredClients.length > displayedClients.length && (
                <button
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-white py-3 text-sm font-bold text-slate-700 shadow-sm"
                >
                  Voir plus
                </button>
              )}
