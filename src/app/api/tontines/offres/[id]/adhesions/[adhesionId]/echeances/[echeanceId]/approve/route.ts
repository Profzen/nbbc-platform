...
import { markEcheanceAsPaid } from '@/lib/tontine-schedule';
...
export async function POST(
  ...
  try {
    ...
    markEcheanceAsPaid(echeance);
    echeance.validatedAt = new Date();
    echeance.validatedBy = (session.user as any)?.id;
    echeance.validationNote = note || 'Échéance validée par un agent.';
    await echeance.save();

    return NextResponse.json({ success: true, data: { echeance } });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message || 'Validation impossible.' }, { status: 500 });
  }
}